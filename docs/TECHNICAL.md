...
v2.0 新增前后端分离的 Web 交互界面，基于 React + FastAPI 实现。

### 8.1 前端架构（`frontend/src/`）

```
frontend/src/
├── App.tsx                   # 路由配置（HashRouter）
├── api/
│   └── client.ts             # Axios API 客户端（类型安全）
├── store/
│   └── useAppStore.ts       # Zustand 全局状态管理
├── pages/
│   ├── Dashboard.tsx         # 仪表盘（统计概览）
│   ├── ArticleList.tsx       # 文章列表（分页、过滤、搜索）
│   ├── ArticleDetail.tsx     # 文章详情（全文格式显示、AI 分析结果）
│   ├── ConceptGraph.tsx     # 概念图谱（vis-network/standalone）
│   ├── Evolution.tsx        # 演化追踪（Recharts 折线图）
│   ├── ConceptList.tsx      # 概念列表（分页、搜索）
│   └── CrossTheory.tsx     # 跨理论对比
├── theme/                   # MUI 主题配置
└── main.tsx                 # 入口文件
```

**路由设计**（基于 `window.location.hash`）：
...
| `#/concept-graph` | `ConceptGraph` | 全局概念图谱 |
| `#/concept-graph?articleId=:id` | `ConceptGraph` | 单篇文章概念子图 |
| `#/evolution` | `Evolution` | 概念演化追踪 |
| `#/concepts` | `ConceptList` | 概念列表 |
| `#/cross-theory` | `CrossTheory` | 跨理论对比 |
...
interface AppState {
  ...
  conceptGraph: ConceptGraphResponse | null;
  evolutionData: EvolutionData | null;
  concepts: Concept[];
  pillarsDistribution: PillarsDistribution | null;
  ...
  fetchConcepts: (params: ConceptParams) => Promise<void>;
...
FastAPI 应用入口：`src/api/app.py`，路由定义在 `src/api/routes.py`。

**API 端点完整列表**：

| 端点 | 方法 | 查询参数 | 说明 |
|------|------|-----------|------|
| `/api/status` | GET | — | 系统状态（文章总数、分析进度、支柱分布） |
| `/api/articles` | GET | `page`, `page_size`, `q`（搜索） | 文章列表（分页） |
| `/api/articles/{id}` | GET | — | 单篇文章详情（含完整 `content_text`） |
| `/api/concept-graph` | GET | `top_n`（默认100）, `article_id` | 概念图谱数据；`article_id` 过滤单篇文章概念子图 |
| `/api/evolution` | GET | — | 概念演化数据（按时间线） |
| `/api/concepts` | GET | `page`, `page_size`, `q` | 概念列表（分页、搜索） |
| `/api/pillars/distribution` | GET | — | 理论支柱分布统计 |
| `/api/cross-theory` | GET | — | 跨理论对比数据 |
...
class ArticleResponse(BaseModel):
    ...
    content_text: str          # 完整全文，未被截断
    crawl_status: str
    analysis_status: str

class ConceptGraphResponse(BaseModel):
    nodes: list[dict]   # [{id, label, frequency, pillar}]
    edges: list[dict]   # [{source, target, weight}]

class EvolutionDataResponse(BaseModel):
    timeline: list[dict]  # [{date, concept, frequency}]
```

### 8.3 概念子图过滤实现

当 URL 包含 `articleId` 参数时，`ConceptGraph.tsx` 会将该参数传递给 API：

```
前端 URL: #/concept-graph?articleId=6
    ↓
ConceptGraph.tsx: 解析 URLSearchParams → articleId = "6"
    ↓
useAppStore.fetchConceptGraph(topN, "6")
    ↓
api/client.ts: GET /api/concept-graph?top_n=100&article_id=6
    ↓
routes.py: get_concept_graph(article_id=6)
    ↓
从 analysis_results 表读取 article_id=6 的 concepts 列表
构建仅包含这些概念的子图（含它们之间的关系边）
```

**注意**：当前实现仅包含有关系连接的概念对。若文章的概念之间无共现关系，子图可能为空。未来计划将文章的所有概念作为孤立节点纳入子图。

### 8.4 前端构建与部署

**已知兼容性问题**：Vite 8 开发服务器（`vite dev`）与 MUI 5 + Emotion 11 存在依赖预构建卡死问题。解决方案：

```bash
# 开发/生产统一使用 build + preview 模式
cd frontend
npm run build           # 输出到 dist/
npm run preview         # 启动静态文件服务器（端口 3000）
```

`vite.config.ts` 中 `preview.proxy` 与 `server.proxy` 配置相同，确保 `vite preview` 也能代理 `/api` 请求到后端。

### 8.5 技术选型说明

| 技术 | 选型原因 |
|------|---------|
| **React 18 + TypeScript** | 类型安全、Vite 原生支持、生态成熟 |
| **MUI v5 + Emotion** | 开箱即用的 Material Design 组件，适合数据展示类应用 |
| **Zustand** | 轻量于 Redux，类型安全于 Context API，内置异步 action 支持 |
| **vis-network/standalone** | `react-vis-network-graph` 有 ESM 兼容问题；直接使用 standalone 版本稳定可靠 |
| **FastAPI** | 自动生成 OpenAPI 文档、Pydantic 数据校验、异步原生支持 |
| **HashRouter** | 支持静态文件部署（无需服务端路由），`vite preview` 可直接使用 |
